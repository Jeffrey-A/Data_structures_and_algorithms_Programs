...
class BST:
    ...
    def preOrder_gen(self):
        ...
        if node is not None:
            yield node.item
            while leftside.left is not None:
                self.help(leftside)
                leftside = leftside.left
                
##            while rightside.right is not None:
##                rightside = rightside.right
##                if rightside.left.left is not None:
##                    yield rightside.left.left.item
                
                    
            
   
    def postOrder_gen(self):
         """postorder traversal."""
    ...
    #Exercise --> 5            
    def __len__(self):
        """It calculates how many items are in the tree.
        post: returns the number of items in the tree."""
    ...
    def rec_find(self,item):
        ...
        elif self.root.item == item: #Base case
                return item
        else:
            while self.root != None and self.root.item != item:
                if item < self.root.item:
                    self.root= self.root.left #left child is the new root
                else:
                    self.root= self.root.right #right child is the new root
            if self.root !=None:#With out this condition I get an attribute error when: self.rec_find(self.root.item)
                return self.rec_find(self.root.item) #Recursive call
